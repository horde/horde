...
(window.gloader || glow).module({
	name: "glow.widgets",
	library: ["glow", "1.5.1"],
	depends: [["glow", "1.5.1", 'glow.dom', 'glow.events']],
	builder: function(glow) {
		...
		glow.ready(function() {
			...
			var testDiv = glow.dom.create('<div class="glow151-cssTest" style="height:20px;position:absolute;visibility:hidden;top:-20px;display:block"></div>').appendTo(docBody);
			
			if (testDiv.height() != 20) {
				//css disabled
				docBody.className += " glow151-basic";
			} else {
				(function() {
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		var dom = glow.dom,
			...
			*/
			overlayHtml = '<div class="glow151-overlay glowNoMask"><div class="overlay-focalPoint" tabindex="-1"></div></div>',
		...
		*/
		function Overlay(content, opts) {
			...
			this.opts = opts = glow.lang.apply({
				...
				focusOnShow: false,
				id: "glow151Overlay" + (++overlayCount)
			}, opts);
	...
	builder: function(glow) {
			...
			glow.ready(function () {
				var widgetSheet = function (sheets) {
					...
					_ieCssRule = function(theme, className, image) {
						return ".glow151-ie .glow151-overlay" + cssPngThemes[theme].className + " ." +
		...
		*/
		defaultTemplate = function() {
			var r = [], rLen = 0;
			r[rLen++] = '<div class="glow151-panel">';
		...
		*/
		function Panel(content, opts) {
			...
			if (!customTemplate) {
				...
				if (!themesPreloaded[opts.theme] && docBody.className.indexOf("glow151-basic") == -1) {
					fullContentClone = fullContent.clone().addClass("glow151-panel-preload").appendTo(docBody);
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		var dom = glow.dom,
			...
			env = glow.env,
			win,
			positionRegex = /glow151\-infoPanel\-point[TRBL]/,
		...
		*/
		function InfoPanel(content, opts) {
			...
			if (!customTemplate) {
				this.content.addClass("glow151-infoPanel");
			}

			this.content.addClass("glow151-infoPanel-point" + (opts.pointerPosition || "t").slice(0,1).toUpperCase());
		}
			...
			setPosition: function(x, y) {
				...
				if (!pointerPosition) {
					...
					if (lastPointerPosition != pointerPosition) {
						...
						contentNode.className = contentNode.className.replace(positionRegex, "glow151-infoPanel-point" + pointerPosition);
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		...
		var $ = glow.dom.get,
			...
			/*
			 HTML for the slider 
			 This still needs to be wrapped in <div class="glow151-slider"> or
			 <div class="glow151-vSlider"> before it's created
			*/
			SLIDER_TEMPLATE = '' +
		...
		(function() {
			...
			*/
			Ruler = function(container, opts) {
				...
				// create element
				this.element = element = glow.dom.create('<div role="presentation" class="glow151-' + vocab.containerClassNamePart + '"><div class="ruler-spacer"></div><div class="ruler-labels"></div></div>');
		...
		*/
		...
		Slider = glow.widgets.Slider = function(container, opts) {
			...
			*/
			...
			this.element = element = glow.dom.create('<div class="glow151-' + vocab.containerClassNamePart + '" tabindex="0" role="slider" aria-disabled="false">' + SLIDER_TEMPLATE + '</div>');
...
/* private functions **********************************************************/
		...
		function getParentListItem(that, node) { /*debug*///console.log("getParentListItem("+node+")");
			...
			while (listItem.parentNode && listItem.parentNode.parentNode) {
				if ($(listItem.parentNode.parentNode).hasClass('glow151-autoSuggest')) break;
		...
		*/
		glow.widgets.AutoSuggest = function(inputElement, dataSource, opts) { /*debug*///console.log('new glow.widgets.AutoSuggest('+inputElement+', '+dataSource+', '+opts+')');
			...
			this.overlay = new glow.widgets.Overlay(
				glow.dom.create('<div class="glow151-autoSuggest"><ul></ul></div>'),
				{
					autoPosition: false,
...
/* public methods *************************************************************/
		...
		glow.widgets.AutoSuggest.prototype.configure = function(opts) {
			...
			if (this.opts.height) {
				var listContainer = $(this.overlay.container.get('.glow151-autoSuggest').get('ul')[0]);
...
//			opts.step = Number(opts.step);
			...
			this.element =
				dom.create("<div"
					...
					+ " class=\"" + (this._opts.vertical? "glow151-vCarousel" : "glow151-carousel")
			...
			if (this._opts.vertical) {
				this.element.addClass("glow151-vCarousel");
			}
			else {
				this.element.addClass("glow151-carousel");
			}
	...
	builder: function(glow) {
		...
		*/
		...
		glow.widgets.Editor = function(textarea, opts) {
			...
			this.element = glow.dom.create('<div class="glow151-editor"><p class="glow151-hidden">You are about to enter a Rich Text Editor, <a href="#endOfEditor' + endOfEditorCounter() + '" tabindex="0">Skip past</a></p><div class="editor-' + (opts.theme || "light") + '"><div class="editor-state"></div></div><p id="endOfEditor' + endOfEditorCounter() + '" class="glow151-hidden endOfEditorCounter" tabindex="0">You have left the Rich Text Editor</p></div>');
