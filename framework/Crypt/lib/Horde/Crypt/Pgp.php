    ...
    public function pgpPrettyKey($pgpdata)
    {
        ...
        foreach ($info['signature'] as $uid_idx => $val) {
            ...
            $msg .= $leftrow[0] . (isset($key['name']) ? stripcslashes($key['name']) : '') . "\n"
                ...
                . $leftrow[2] . (empty($key['keyid']) ? Horde_Crypt_Translation::t("Unknown") : strftime("%D", $val[$sig_key]['created'])) . "\n"
                . $leftrow[3] . (empty($key['keyid']) ? Horde_Crypt_Translation::t("Unknown") : (empty($val[$sig_key]['expires']) ? '[' . Horde_Crypt_Translation::t("Never") . ']' : strftime("%D", $val[$sig_key]['expires']))) . "\n"
