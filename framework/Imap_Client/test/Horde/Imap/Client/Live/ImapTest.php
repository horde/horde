...
class Horde_Imap_Client_Live_ImapTest extends Horde_Test_Case
{
    ...
    {
        $config = self::getConfig('IMAPCLIENT_TEST_CONFIG', __DIR__ . '/../');
        if (is_null($config) || empty($config['imapclient']['enabled'])) {
            self::$error = 'IMAP server test not enabled.';
            return;
        }
        ...
        self::$created = false;
        self::$error = null;
        self::$test_mbox = $config['imapclient']['test_mbox'];
        self::$test_mbox_utf8 = $config['imapclient']['test_mbox_utf8'];
        ...
        self::$imap = new Horde_Imap_Client_Socket(
            $config['imapclient']['client_config']
        );
    }
