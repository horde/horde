...
class Horde_Core_Ui_VarRenderer_Html extends Horde_Core_Ui_VarRenderer
{
    ...
    protected function _renderVarInputDefault($form, &$var, &$vars)
    {
        return '<strong>Warning:</strong> Unknown variable type ' .
            @htmlspecialchars($var->getTypeName(), ENT_QUOTES, 'UTF-8');
    }
    ...
    protected function _renderVarInput_number($form, &$var, &$vars)
    {
        ...
        $varname = @htmlspecialchars($var->getVarName(), ENT_QUOTES, 'UTF-8');
    ...
    protected function _renderVarInput_stringarray($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="text" size="60" name="%s" id="%s" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars(implode(', ', $var->getValue($vars)), ENT_QUOTES, 'UTF-8'),
                       $this->_getActionScripts($form, $var)
               );
    }
    ...
    protected function _renderVarInput_monthdayyear($form, &$var, &$vars)
    {
        ...
        foreach ($date_parts as $part) {
            $varname = @htmlspecialchars($var->getVarName() . '[' . $part . ']', ENT_QUOTES, 'UTF-8');
    ...
    {
        $varname = @htmlspecialchars($var->getVarName(), ENT_QUOTES, 'UTF-8');
        $value = @htmlspecialchars($var->getValue($vars), ENT_QUOTES, 'UTF-8');
        ...
        foreach ($var->type->getSounds() as $sound) {
            $sound = @htmlspecialchars($sound, ENT_QUOTES, 'UTF-8');
    ...
    protected function _renderVarInput_assign($form, &$var, &$vars)
    {
        ...
        $name = @htmlspecialchars($var->getVarName(), ENT_QUOTES, 'UTF-8');
    ...
    protected function _renderVarInput_enum($form, &$var, &$vars)
    {
        ...
        if (!empty($prompt)) {
            $prompt = '<option value="">' . ($htmlchars ? $prompt : @htmlspecialchars($prompt, ENT_QUOTES, 'UTF-8')) . '</option>';
        }
        $varname = @htmlspecialchars($var->getVarName(), ENT_QUOTES, 'UTF-8');
    ...
    protected function _renderVarInput_mlenum($form, &$var, &$vars)
    {
        ...
        $hvarname = @htmlspecialchars($varname, ENT_QUOTES, 'UTF-8');
        $values = $var->getValues();
    ...
    protected function _renderVarInput_multienum($form, &$var, &$vars)
    {
        ...
        return sprintf('<select multiple="multiple" size="%s" name="%s[]" %s>%s</select>',
                       (int)$var->type->size,
                       @htmlspecialchars($var->getVarName(), ENT_QUOTES, 'UTF-8'),
                       $this->_getActionScripts($form, $var),
...
EOT;
    ...
    protected function _renderVarInput_emailconfirm($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="email" name="%s[original]" id="%s[original]" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars($email['original'], ENT_QUOTES, 'UTF-8'),
                       $this->_getActionScripts($form, $var)) .
            ' ' . sprintf('<input type="email" name="%s[confirm]" id="%s[confirm]" value="%s"%s />',
                          $varname,
                          $varname,
                          @htmlspecialchars($email['confirm'], ENT_QUOTES, 'UTF-8'),
                          $this->_getActionScripts($form, $var));
    }
    ...
    protected function _renderVarInput_passwordconfirm($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="password" name="%s[original]" id="%s[original]" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars($password['original'], ENT_QUOTES, 'UTF-8'),
                       $this->_getActionScripts($form, $var)) .
            ' ' . sprintf('<input type="password" name="%s[confirm]" id="%s[confirm]" value="%s"%s />',
                          $varname,
                          $varname,
                          @htmlspecialchars($password['confirm'], ENT_QUOTES, 'UTF-8'),
                          $this->_getActionScripts($form, $var));
    }
    ...
    protected function _renderVarInput_obrowser($form, &$var, &$vars)
    {
        ...
        $html .= sprintf('<input type="hidden" name="%s" id="%s"%s value="%s">',
                         @htmlspecialchars($varname, ENT_QUOTES, 'UTF-8'),
                         $fieldId,
                         $this->_getActionScripts($form, $var),
                         @htmlspecialchars($varvalue, ENT_QUOTES, 'UTF-8'));
        if (!empty($varvalue)) {
            $html .= $varvalue;
        }
    ...
    {
        return nl2br(@htmlspecialchars($var->getValue($vars), ENT_QUOTES, 'UTF-8'));
    }
    ...
    protected function _renderVarDisplay_email($form, &$var, &$vars)
    {
        ...
        if ($var->type->getProperty('link_compose')) {
            ...
            foreach ($addrs as $addr) {
                ...
                $link .= Horde::link($mail_link, $addr) . @htmlspecialchars($display_email, ENT_QUOTES, 'UTF-8') . '</a>';
            }
            ...
            return nl2br(@htmlspecialchars($email_val, ENT_QUOTES, 'UTF-8'));
        }
    }
    ...
    protected function _renderVarDisplay_phone($form, &$var, &$vars)
    {
        ...
        $html = @htmlspecialchars($number, ENT_QUOTES, 'UTF-8');
    ...
    protected function _renderVarDisplay_address($form, &$var, &$vars, $text = true)
    {
        ...
        $html = $text ? nl2br(@htmlspecialchars($address, ENT_QUOTES, 'UTF-8')) : '';
        if (!empty($mapurl)) {
    ...
    {
        return '<span class="form-error">' . @htmlspecialchars($var->type->message, ENT_QUOTES, 'UTF-8') . '</span>';
    }
    ...
    protected function _renderVarDisplay_link($form, &$var, &$vars)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $html .= Horde::link($values[$i]['url'], $values[$i]['text'], 'widget', $values[$i]['target'], $values[$i]['onclick'], $values[$i]['title'], $values[$i]['accesskey']) . @htmlspecialchars($values[$i]['text'], ENT_QUOTES, 'UTF-8') . '</a>';
        }
            ...
            if ($files) {
                ...
                foreach ($files as $id => $file) {
                    ...
                    if ($GLOBALS['registry']->hasMethod('files/getViewLink')) {
                        ...
                        $filename = Horde::link($url, Horde_Core_Translation::t("Preview"), null, 'form_file_view') . @htmlspecialchars(Horde_Util::realPath($dir . '/' . $filename), ENT_QUOTES, 'UTF-8') . '</a>';
                    } else {
                        if (!empty($dir) && ($dir != '.')) {
    ...
    protected function _multiSelectOptions(&$values, $selectedValues)
    {
        ...
        foreach ($values as $value => $display) {
            ...
            $result .= " <option value=\"" . @htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . "\"$selected>" . @htmlspecialchars($display, ENT_QUOTES, 'UTF-8') . "</option>\n";
        }
