...
class Horde_Core_Ui_VarRenderer_Html extends Horde_Core_Ui_VarRenderer
{
    ...
    protected function _renderVarInputDefault($form, &$var, &$vars)
    {
        return '<strong>Warning:</strong> Unknown variable type ' .
            @htmlspecialchars($var->getTypeName(), ENT_QUOTES);
    }
    ...
    protected function _renderVarInput_int($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="number" size="5" name="%s" id="%s" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars($var->getValue($vars), ENT_QUOTES),
                       $this->_getActionScripts($form, $var)
               );
    }
    ...
    protected function _renderVarInput_stringarray($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="text" size="60" name="%s" id="%s" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars(implode(', ', $var->getValue($vars)), ENT_QUOTES),
                       $this->_getActionScripts($form, $var)
               );
    }
    ...
    protected function _renderVarInput_monthdayyear($form, &$var, &$vars)
    {
        ...
        foreach ($date_parts as $part) {
            $varname = @htmlspecialchars($var->getVarName() . '[' . $part . ']', ENT_QUOTES);
    ...
    {
        $varname = @htmlspecialchars($var->getVarName(), ENT_QUOTES);
        $value = @htmlspecialchars($var->getValue($vars), ENT_QUOTES);
        ...
        foreach ($var->type->getSounds() as $sound) {
            $sound = @htmlspecialchars($sound, ENT_QUOTES);
    ...
    protected function _renderVarInput_assign($form, &$var, &$vars)
    {
        ...
        $name = @htmlspecialchars($var->getVarName(), ENT_QUOTES);
    ...
    protected function _renderVarInput_enum($form, &$var, &$vars)
    {
        ...
        if (!empty($prompt)) {
            $prompt = '<option value="">' . ($htmlchars ? $prompt : @htmlspecialchars($prompt, ENT_QUOTES)) . '</option>';
        }
        $varname = @htmlspecialchars($var->getVarName(), ENT_QUOTES);
    ...
    protected function _renderVarInput_mlenum($form, &$var, &$vars)
    {
        ...
        $hvarname = @htmlspecialchars($varname, ENT_QUOTES);
        $values = $var->getValues();
        ...
        $html = sprintf('<input type="hidden" name="%s[old]" value="%s" %s />',
                        $hvarname,
                        @htmlspecialchars($selected['1'], ENT_QUOTES),
        ...
        if (!empty($prompts)) {
            $html .= '<option value="">' . @htmlspecialchars($prompts[0], ENT_QUOTES) . '</option>';
        }
        ...
        $html .= sprintf('<select %s name="%s[2]"%s>',
                         ...
                         ($var->hasAction() ? ' ' . $this->_genActionScript($form, $var->_action, $varname) : ''));
        if (!empty($prompts)) {
            $html .= '<option value="">' . @htmlspecialchars($prompts[1], ENT_QUOTES) . '</option>';
        }
        $values_2 = array();
        if (!empty($selected['1'])) {
    ...
    protected function _renderVarInput_multienum($form, &$var, &$vars)
    {
        ...
        return sprintf('<select multiple="multiple" size="%s" name="%s[]" %s>%s</select>',
                       (int)$var->type->size,
                       @htmlspecialchars($var->getVarName(), ENT_QUOTES),
                       $this->_getActionScripts($form, $var),
...
EOT;
    ...
    protected function _renderVarInput_emailconfirm($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="email" name="%s[original]" id="%s[original]" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars($email['original'], ENT_QUOTES),
                       $this->_getActionScripts($form, $var)) .
            ' ' . sprintf('<input type="email" name="%s[confirm]" id="%s[confirm]" value="%s"%s />',
                          $varname,
                          $varname,
                          @htmlspecialchars($email['confirm'], ENT_QUOTES),
                          $this->_getActionScripts($form, $var));
    }
    ...
    protected function _renderVarInput_passwordconfirm($form, &$var, &$vars)
    {
        ...
        return sprintf('<input type="password" name="%s[original]" id="%s[original]" value="%s"%s />',
                       $varname,
                       $varname,
                       @htmlspecialchars($password['original'], ENT_QUOTES),
                       $this->_getActionScripts($form, $var)) .
            ' ' . sprintf('<input type="password" name="%s[confirm]" id="%s[confirm]" value="%s"%s />',
                          $varname,
                          $varname,
                          @htmlspecialchars($password['confirm'], ENT_QUOTES),
                          $this->_getActionScripts($form, $var));
    }
    ...
    protected function _renderVarInput_obrowser($form, &$var, &$vars)
    {
        ...
        $html .= sprintf('<input type="hidden" name="%s" id="%s"%s value="%s">',
                         @htmlspecialchars($varname, ENT_QUOTES),
                         $fieldId,
                         $this->_getActionScripts($form, $var),
                         @htmlspecialchars($varvalue, ENT_QUOTES));
        if (!empty($varvalue)) {
            $html .= $varvalue;
        }
    ...
    protected function _renderVarDisplay_email($form, &$var, &$vars)
    {
        ...
        if ($var->type->getProperty('link_compose')) {
            ...
            foreach ($addrs as $addr) {
                ...
                $link .= Horde::link($mail_link, $addr) . @htmlspecialchars($display_email, ENT_QUOTES) . '</a>';
            }
            ...
            return nl2br(@htmlspecialchars($email_val, ENT_QUOTES));
        }
    }
    ...
    protected function _renderVarDisplay_enum($form, &$var, &$vars)
    {
        ...
        if (count($values) == 0) {
            ...
            return @htmlspecialchars($values[$value], ENT_QUOTES);
        }
    }
    ...
    protected function _renderVarDisplay_radio($form, &$var, &$vars)
    {
        ...
        if (count($values) == 0) {
            ...
            return @htmlspecialchars($values[$var->getValue($vars)], ENT_QUOTES);
        }
    }
    ...
    protected function _renderVarDisplay_multienum($form, &$var, &$vars)
    {
        ...
        if (!count($values) || !count($on)) {
            ...
            return @htmlspecialchars(implode(', ', $display), ENT_QUOTES);
        }
    }
    ...
    protected function _renderVarDisplay_phone($form, &$var, &$vars)
    {
        ...
        $html = @htmlspecialchars($number, ENT_QUOTES);
    ...
    protected function _renderVarDisplay_address($form, &$var, &$vars, $text = true)
    {
        ...
        $html = $text ? nl2br(@htmlspecialchars($address, ENT_QUOTES)) : '';
        if (!empty($mapurl)) {
    ...
    {
        return '<span class="form-error">' . @htmlspecialchars($var->type->message, ENT_QUOTES) . '</span>';
    }
    ...
    protected function _renderVarDisplay_link($form, &$var, &$vars)
    {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $html .= Horde::link($values[$i]['url'], $values[$i]['text'], 'widget', $values[$i]['target'], $values[$i]['onclick'], $values[$i]['title'], $values[$i]['accesskey']) . @htmlspecialchars($values[$i]['text'], ENT_QUOTES) . '</a>';
        }
            ...
            if ($files) {
                ...
                foreach ($files as $id => $file) {
                    ...
                    if ($GLOBALS['registry']->hasMethod('files/getViewLink')) {
                        ...
                        $filename = Horde::link($url, Horde_Core_Translation::t("Preview"), null, 'form_file_view') . @htmlspecialchars(Horde_Util::realPath($dir . '/' . $filename), ENT_QUOTES) . '</a>';
                    } else {
                        if (!empty($dir) && ($dir != '.')) {
    ...
    protected function _multiSelectOptions(&$values, $selectedValues)
    {
        ...
        foreach ($values as $value => $display) {
            ...
            $result .= " <option value=\"" . @htmlspecialchars($value, ENT_QUOTES) . "\"$selected>" . @htmlspecialchars($display, ENT_QUOTES) . "</option>\n";
        }
