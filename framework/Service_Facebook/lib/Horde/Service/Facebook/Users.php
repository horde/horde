...
class Horde_Service_Facebook_Users extends Horde_Service_Facebook_Base
{
    ...
    /**
     * Sets the user's current status message.
     *
     ...
     * @return boolean
     */
    public function postStatus($status, $uid = null)
    {
        if (empty($uid) && !$this->_facebook->auth->getSessionKey()) {
        ...
        return $this->_facebook->callGraphApi(
            $uid . '/feed', $params, array('request' => 'POST'));
    }
     ...
     * @param integer $limit
     *
     * @return object  A stdClass containing data and paging properties.
     *
     */
    public function getStatus($uid = null, $limit = 1)
    {
        if (empty($uid) && !$this->_facebook->auth->getSessionKey()) {
            throw new Horde_Service_Facebook_Exception('users.getStatus requires a uid or a session_key',
                Horde_Service_Facebook_ErrorCodes::API_EC_PARAM_SESSION_KEY);
        }
        if (empty($uid)) {
            $uid = 'me';
        ...
        $results = $this->_facebook->callGraphApi($uid . '/feed', $params);
    }
