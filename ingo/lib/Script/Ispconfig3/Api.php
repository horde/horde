...
class Ingo_Script_Ispconfig3_Api
{
    ...
    public function processRules($rules)
    {
        try
        {
            ...
            {
                $status = empty($rule['disable']) ? 'y' : 'n';

                switch ($rule['action'])
                {
                    case Ingo_Storage::ACTION_BLACKLIST:
                        $rule = $this->_params['storage']
     ...
     * 
     * @param array $rule Ingo rule
     * @param string $status Disable state
     * @param string $filter Filter type (W/B)
     */
    protected function _wblist($rule, $status, $filter)
    {
        $this->_getSpamUser();
        ...
        else
        {
            ...
            $wb_list = $rule->getBlacklist();
            $type = 'blacklist';
        }

        // No action if no filters.
        if (empty($wb_list) && empty($api_list))
        {
            return;
        }
        ...
        if (!empty($api_list))
        {
            ...
            foreach ($api_list as $api_id => $api_check)
            {
                if ($wb_list)
                {
                    $candidate = array_search($api_check['email'], $wb_list);
     ...
     * @param string $status Disable state
     */
    protected function _vacation($rule, $status)
    {
        $start = $rule->getVacationStart();
        $end = $rule->getVacationEnd();
        ...
        $subject = $rule->getVacationSubject();

        if ($this->_mail_user['autoresponder_subject'] != $subject)
        {
            $params['autoresponder_subject'] = $subject;
        }

        $text = Ingo_Script_Util::vacationReason(
            $rule->getVacationReason(),
            $start,
            $end
        );
     ...
     * @param string $status Disable state
     */
    protected function _forward($rule, $status)
    {
        $action = array();

        $addresses = $rule->getForwardAddresses();

        if ($status == 'y' && !empty($addresses))
        {
     ...
     */
    protected function _filter($rule, $status, $action)
    {
        $target = $action == 'delete' ? '' : $rule['action-value'];
