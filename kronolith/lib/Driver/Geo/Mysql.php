 ...
 * @package Kronolith
 */
class Kronolith_Driver_Geo_Mysql extends Kronolith_Driver_Geo_Sql
{
    ...
    public function setLocation($event_id, $point)
    {
        ...
        /* INSERT or UPDATE */
        if ($count) {
            $sql = "UPDATE kronolith_events_geo SET coordinates = GeomFromText('POINT(" . $point['lat'] . " " . $point['lng'] . ")') WHERE event_id = '" . $event_id . "'";
        } else {
            $sql = "INSERT into kronolith_events_geo (event_id, coordinates) VALUES('" . $event_id . "', GeomFromText('POINT(" . $point['lat'] . " " . $point['lng'] . ")'))";
        }
    ...
    {
        $sql = "SELECT x(coordinates) as lat, y(coordinates) as lng FROM kronolith_events_geo WHERE event_id = '" . $event_id . "'";
