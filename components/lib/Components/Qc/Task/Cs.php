...
extends Components_Qc_Task_Base
{
    ...
    {
        $lib = realpath(getcwd() . '/' . $this->_config->getPath() . '/lib');
        $argv = $_SERVER['argv'];
        $argc = $_SERVER['argv'];
        $_SERVER['argv'] = array();
        $_SERVER['argc'] = 0;
        ...
        $numErrors = $reporting->printReport('emacs', $filesViolations, false, null);
        if ($numErrors) {
            $this->getOutput()->warn("$numErrors errors!");
        } else {
            $this->getOutput()->ok('No problems found.');
        }
    }
}